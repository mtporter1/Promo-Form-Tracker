...
# Handle row deletion outside form
for i in range(len(st.session_state.eu3_rows)):
    if st.button(f"❌ Remove Row {i+1}", key=f"delete_row_{i}"):
        st.session_state.eu3_rows.pop(i)
...
with st.form("promo_form"):
    ...
    for i, row in enumerate(st.session_state.eu3_rows):
        ...
        row['reseller'] = c1.text_input("Reseller", value=row['reseller'], key=f"reseller_{i}")
        row['support'] = c2.text_input("Margin Support", value=row['support'], key=f"support_{i}")
