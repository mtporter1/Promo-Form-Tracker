...
with st.form("promo_form"):
    ...
    eu3_rows = st.session_state.get("eu3_rows", 1)

    cols = st.columns([0.45, 0.45, 0.1])
    if cols[2].button("+ Add", use_container_width=True):
        st.session_state.eu3_rows = eu3_rows + 1

    eu3_data = []
    for i in range(eu3_rows):
        c1, c2 = st.columns([0.5, 0.5])
        reseller = c1.text_input(f"Reseller {i+1}", key=f"reseller_{i}")
        support = c2.text_input(f"Margin Support per Unit {i+1}", key=f"support_{i}")
